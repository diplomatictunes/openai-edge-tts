...
async def _get_voices(language=None):
    """
    Retrieves and filters Edge TTS voices based on specified language(s).
    The 'language' parameter can be:
    - None or empty: uses DEFAULT_LANGUAGE from config.
    - 'all': returns all available voices.
    - A single locale string (e.g., 'en-US', 'en-AU').
    - A comma-separated string of locales (e.g., 'en-US,en-AU').
    - A general language code (e.g., 'en') to include all country-specific variations.
    """
    all_voices = await edge_tts.list_voices()
    
    # Use default if no language specified, then split into individual locales
    target_locales_str = language or DEFAULT_LANGUAGE
    # Create a list of target locales, stripping whitespace
    target_locales = [locale.strip() for locale in target_locales_str.split(',') if locale.strip()]

    filtered_voices = []
    for v in all_voices:
        # Check if 'all' is in the target_locales, meaning return all voices
        if 'all' in target_locales:
            filtered_voices.append({"name": v['ShortName'], "gender": v['Gender'], "language": v['Locale']})
        # Check for exact locale matches (e.g., 'en-US' == 'en-US')
        elif v['Locale'] in target_locales:
            filtered_voices.append({"name": v['ShortName'], "gender": v['Gender'], "language": v['Locale']})
        # Check if a general language code is in target_locales and the voice's locale starts with it
        # E.g., if 'en' is in target_locales, include 'en-US', 'en-AU', 'en-GB'
        elif any(v['Locale'].startswith(f"{gen_lang}-") for gen_lang in target_locales if len(gen_lang) == 2):
            filtered_voices.append({"name": v['ShortName'], "gender": v['Gender'], "language": v['Locale']})
